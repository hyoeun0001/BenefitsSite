...
//검색어('key') 입력시 주관부서,혜택명에 맞는 데이터 가져오기  
$searchword = $_GET['key'];
$sc = "SELECT * from benefit where BENEFITNAME like '%$searchword%' or DIC like '%$searchword%' order by bn";
     ...
     <div id="container">
       ...
       <main class="contents">
         ...
         <table class="benefit_table">
             ...
             <tbody>
             <?php 
              for($i=0; $i<$row; $i++ ){  //for문으로 순서대로 sql실행된것을 출력
               if (count($array) >= 1)  {
                 echo ('
                <tr>
                  <td> ' . $array[$i]["BN"] . ' </td>
