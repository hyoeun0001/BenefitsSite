    ...
    if(empty($name))
    {
        ...
        if($row['ID'] == $id ) {
            ...
            if ($result) {
                ...
                // $sqll = "select* from benefit";
                // $rec = oci_parse($db, $sqll);
                // oci_execute($rec);

                // $res = array(); //혜택 id
                // $row_num = oci_fetch_all($rec, $res, 0, -1, OCI_FETCHSTATEMENT_BY_ROW);
                ...
                //     //array($id => $res[$i]["BN"]);
                //    // $bn=$res[$i]["BN"];
                //    // $sqlu = "insert into C_B(id, bn) values('$id', '$bn')";
                   
                //     $ret = oci_parse($db, $sqlu);
                //     oci_execute($ret);
